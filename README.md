...
**图文攻略编辑器**
章节 → 次级成就两级结构，两级都可拖拽排序和跨章节移动；富文本编辑（标题 H1 / H2 / H3、加粗、斜体、下划线、删除线、对齐、列表）；截图直接插入正文并可拖动排序；图片上下方可插入**题注**并绑定到该图片；次级成就有分类标签和备注。

**成就分类按游戏独立**
每个游戏一套分类和配色，可随时改名、改色、增删，不同游戏之间互不干扰。

**攻略浏览**
纵向时间线 + 顶部路线图导航；已解锁成就为金色名称 + 图标外圈转动的彩虹环，未解锁的则后退；章节全部完成时描边变金色。右上角按钮三态循环：**全部展开 → 未完成 → 全部折叠**——「未完成」把拿到手的滤掉，屏幕上只剩还要做的。

**新解锁的成就，页面自己滚过去** —— 自动同步发现新解锁时，攻略滚到那一条并闪烁提示，一次解锁多个就依次来；悬浮阅读窗会先自己滑出来。

**全成就达成** —— 全部成就解锁后，注意事项栏的封面常驻换成达成卡：彩虹边框 + 金色奖杯。它是页面的一部分而不是弹窗，进多少次都不打扰。
...
**数据导出（游戏库 + 成就）**
把你的游戏库和成就数据写成文件，三种格式任选：**JSON**（缩进排版，署名 Steam 昵称，备份迁移用）、**CSV**（Excel 直接打开不乱码，每行带所属游戏的进度列）、**HTML**（自带搜索 / 排序 / 完成状态筛选的单文件页面，发给没装本软件的人也能看）。
...
**界面风格：霓虹 / 简约**
同一处的第二根拨杆。霓虹是原本的金青发光风格；简约把强调色压成低饱和（金变琥珀、青变灰蓝），撤去光晕和动效。它和深色 / 浅色正交，四个组合都成立、随点随生效。
...
**全局热键**：`Ctrl + Alt + G` 唤出 / 隐藏攻略悬浮窗，`Ctrl + Alt + E` 编辑悬浮窗 —— 游戏全屏时同样生效，且会自动打开**正在玩的游戏**的攻略。热键只做显示 / 隐藏，不关闭窗口。

**不透明度**：工具栏的 % 按钮在 100 / 90 / 80 / 70 之间循环，压在亮色游戏画面上读字更省力，档位跨启动保留。

自动同步带回新解锁时：阅读窗自动弹出、逐条滚过去闪烁；编辑窗弹一条可点击的轻提示，跳不跳由你。
...
当前最新版本 **v0.8.0**（2026-09-25）—— 见 [更新日志](#-更新日志)。
...
一个成就一个卡片，标识成就信息，有图文攻略时，点击卡片弹出图文攻略；攻略里的**图片可以点开看大图**。
...
**深色 / 浅色**两套配色，**霓虹 / 简约**两种强调风格，切换后立即生效。

浅色模式不是简单地把背景刷白 —— 原本的霓虹配色在白底上几乎没有对比度，六个强调色全部换成了同色相压暗、达到可读线的版本。简约风把强调色压成低饱和，撤去光晕和动效。
...
设置里的「数据导出」把你的**游戏库和成就数据**写成文件，JSON / CSV / HTML 三种格式（详见上方「数据导出」）。HTML 是一个自带筛选的单文件页面，适合分享；JSON 适合备份和换机器。
...
| `Ctrl + 滚轮` | 全局                | 缩放界面       |
| `Ctrl + 0`    | 全局                | 恢复 100% 缩放 |
| `Ctrl + Alt + G` | 全局（游戏内也生效） | 唤出 / 隐藏攻略悬浮窗 |
| `Ctrl + Alt + E` | 全局（游戏内也生效） | 唤出 / 隐藏悬浮编辑窗 |
...
**游戏库和成就数据能导出吗？**
能。**设置 → 数据导出**，JSON / CSV / HTML 三种格式任选 —— 详见上方「数据导出」一节。要导出**单条攻略**给别人，用的是游戏详情页 / 攻略页的导出入口（见「导入导出」）。
...
这一版的主线是界面的系统化与不打扰：新增「简约」界面风格、悬浮窗全局热键与透明度，数据导出从占位变成真实现。

**界面风格：霓虹 / 简约** —— 设置 → 外观第二根拨杆，与深色 / 浅色正交。简约把强调色压成低饱和（金变琥珀、青变灰蓝），撤去光晕和动效；主交互蓝全应用统一为编辑器焦点蓝。

**数据导出** —— 游戏库 + 成就数据导出为 **JSON**（缩进排版，署名 Steam 昵称）、**CSV**（带 BOM，每行带游戏进度列）或 **HTML**（自带搜索 / 排序 / 完成状态筛选的单文件页面）。HTML 适合发给没装本软件的人，JSON 适合备份换机。

**悬浮窗全局热键** —— `Ctrl+Alt+G` 攻略悬浮窗，`Ctrl+Alt+E` 编辑悬浮窗。游戏全屏时也生效，自动打开**正在玩的游戏**的攻略；是显隐切换，不关闭窗口。

**悬浮窗不透明度** —— 工具栏 % 按钮，100/90/80/70 循环，跨启动保留。

**全成就达成不再弹屏** —— 庆祝常驻注意事项栏封面位，不再每次进入攻略页打扰。

**修**：双开应用会共享数据库并撞锁；批量同步中关掉悬浮窗会把剩余同步整个中止；悬浮窗拖动时误选文字；旧攻略文档在悬浮编辑窗打开会崩；游戏库查询随同步日志增长变慢。

### v0.7.1 — 2026-09-24

- **未完成筛选** —— 攻略页右上角按钮三态循环：全部展开 / 未完成 / 全部折叠。「未完成」把拿到手的滤掉、整章做完的收成一行
- **新解锁的成就自动滚动高亮** —— 自动同步发现新解锁时滚过去并闪烁，多个依次来，每个停 4 秒；顺序按攻略走，页面只往下不回跳
- **全成就达成庆祝** —— 游戏 100% 后打开攻略弹一张奖杯页
- 安装包瘦身 18 MB（95.8 → 77.4 MB），解包文件数 12,544 → 102

### v0.6.1 — 2026-09-22

这一版没有新界面，重点在**让已有的东西别出错**：同步不再被几个永远拉不动的游戏拖住，编辑器里两个写作框的底边终于对得齐，拖缩略图换顺序不会再吃掉一张图。另外补了两件一直缺的东西 —— 编辑器能写标题了，攻略里的图片可以点开看大图。

**同步**

- **被 Steam 明确拒绝的游戏，不再拖住整个同步** —— 增量的判据过去只有「有没有成功记录」，于是「Steam 永远不给」和「从没同步过、值得一试」在它眼里完全一样，待同步集合被那些游戏占满。实测一个 171 款游戏的库：18 款被拒，而真正有变动的 —— 一个都没进去。
- **隐私资料不再清空已有数据** —— 把 Steam 资料设成私密的游戏，过去会把已经同步好的成就一起清掉。
- **同步结果如实上报** —— 「游戏库」和「成就」是两次不同的请求，现在分开报告，不再用一个笼统的失败盖住另一半。

**游戏库**

- **封面密度可切** —— 一行放几个自己定
- 同步失败的游戏能被筛出来，不用一个一个翻

**图文编辑器**

- **可以写标题了** —— 工具栏加了 H1 / H2 / H3。在这之前整个软件里没有任何标题样式，从别处粘一个 h2 进来，编辑器、攻略页、导出的 HTML 三个地方长得都不一样
- **描述 / 备注框补上了高度上限** —— 以前拖拽把手向上有底、向下没有顶，能一路拖过卡片、拖进下一章
- 章节区的字段排布改成和次级成就区一致，两边的框终于对齐

**攻略阅读与导出**

- **图片可以点开看大图**
- 章节时间线改成一条贯通的轴线，节点落在线上
- 导出的 HTML 补上了「容易错过」的标识和已完成的金色边框

**修掉的手感问题**

- 描述框与图文编辑框的底边对齐
- 删掉了拖动缩略图时那个浮动的大图预览
- **拖缩略图换顺序偶尔会把一张图复制到别的格子上** —— 当图片分散在不同段落里时会触发
- 游戏详情页的封面不再在横版和竖版之间反复闪烁
